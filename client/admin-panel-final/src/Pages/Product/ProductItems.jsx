...
export default function ProductItems() {
  ...
  let productDetails=(id)=>{
      ...
      .then((res) => res.data)
      .then((finalRes) => {
           setProductData(finalRes.productDetails);
           setOrderModal(true)
      });
  }
              ...
              <div className="p-4 md:p-5 space-y-4">
                <div className="grid grid-cols-[22%_45%_27%] gap-10">
                  <div className="border-2 rounded-md shadow-md p-4">
                    <h3>Main Image</h3>
                    <img
                      src={path+productData.productImage}
                      alt=""
                    />
                     <h3>Back Image</h3>
                    <img
                      src={path+productData.productbackImage}
                      alt=""
                    />
                  </div>
                  <div className="flex items-start flex-wrap gap-5 border-2 rounded-md shadow-md p-3">
                    ...
                    {
                      productData.productGallery.map((imagePath)=>{
                        return(
                           <img
                      className="w-36"
                      src={path+imagePath}
                  ...
                  <div className="border-2 rounded-md shadow-md p-3">
                    ...
                    <ul className="space-y-4 mt-8">
                      ...
                      <li className="font-semibold text-[17px]">
                        Color :{" "}
                        <span className="font-normal text-[16px] ">
                          &nbsp;  { productData.productColor.map((obj,index)=> <> {obj.colorName}  {" "}</>  ) }
                        </span>{" "}
                      </li>
                    </ul>
                  </div>
                    ...
                    data.map((obj, index) => {
                      return (
                        <tr className="bg-white border-b">
                          ...
                          <td className="px-6 py-4"> {obj.productName} 
                                  <br/>
                                  <b>Category Name:</b>
                                   {obj.parentCategory.categoryName}  <br/>
                                    <b>Sub Category Name:</b>
                                   {obj.subCategory.subcategoryName}    <br/>
                                     <b>Sub Sub Category Name:</b>
                                   {obj.subsubCategory.subSubcategoryName}  
                             </td>
                          <td className="px-6 py-4">
                            <p className="line-clamp-1 w-[180px]">
                              Lorem ipsum dolor sit amet consectetur adipisicing
                              elit. Harum ipsa consequatur animi rerum at
                              eveniet praesentium explicabo expedita assumenda
                              voluptas maiores nobis,
                            </p>
                            <button
                              onClick={()=>productDetails(obj._id)}
                          ...
                          <td className="px-6 py-4 flex gap-3 mt-6">
                            ...
                            <Link to={`/product/update/${obj._id}`}>
                                <svg
                              fill="gold"
                              className="w-4 h-4"
                              xmlns="http://www.w3.org/2000/svg"
